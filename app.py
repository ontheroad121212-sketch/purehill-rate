...
def render_master_table(current_df, prev_df, ch_name=None, title="", mode="기준"):
    ...
    html += "<table style='width:100%; border-collapse:collapse; font-family:sans-serif; font-size:11px;'><thead><tr style='background:#f9f9f9;'><th rowspan='2' style='border:1px solid #ddd; width:150px;'>객실/프로모션</th>"
...
st.title("🏨 엠버퓨어힐 전략 통합 RMS (전체 10개 객실)")
...
with st.sidebar:
    st.header("📅 데이터 로드/저장")
    comp_date = st.date_input("비교 과거 날짜", value=date.today())
    if st.button("📂 DB 데이터 로드"):
        ...
        with st.expander(f"📦 {ch} 설정"):
            ...
            for r in ALL_ROOMS:
                if st.checkbox(r, value=(r in curr_rooms), key=f"cb_{ch}_{r}"):
                    new_rooms.append(r)
            st.session_state.promotions[ch]["selected_rooms"] = new_rooms
...
if 'today_df' in st.session_state:
    ...
    st.markdown(render_master_table(st.session_state.today_df, prev, title="📈 2. 예약 변화량 (Pick-up)", mode="변화"), unsafe_allow_html=True)
    st.markdown(render_master_table(st.session_state.today_df, prev, title="🔔 3. 판도 변화 (유채색 원색 알림)", mode="판도변화"), unsafe_allow_html=True)
    
    st.divider()
    for ch in st.session_state.channel_list:
        st.markdown(render_master_table(st.session_state.today_df, prev, ch_name=ch, title=f"✅ {ch} 판매가 산출 (판도 컬러 동기화)", mode="판매가"), unsafe_allow_html=True)
