        ...
        for r_idx in [6, 7, 10, 11, 12]:
            ...
            for d_val, av in zip(dates_raw, df_raw.iloc[r_idx, 2:].values):
                ...
                try:
                    ...
                    all_data.append({"Date": d_obj, "RoomID": rid, "Available": av, "Total": tot, "Tag": date_tag})
                except: continue
    
    if all_data:
        full_df = pd.DataFrame(all_data)
        unique_tags = sorted(full_df['Tag'].unique())
        if len(unique_tags) >= 2:
            st.session_state.today_df = full_df[full_df['Tag'] == unique_tags[-1]]
            st.session_state.prev_df = full_df[full_df['Tag'] == unique_tags[-2]]
        else:
