...
# --- 2. 전역 설정 데이터 (색상 및 상수) ---
# 보라색 채도 맵핑: 변화된 결과가 같은 BAR면 같은 색상
CHANGE_COLORS = {
...
if uploaded_files:
    ...
    if all_temp:
        ...
        if len(unique_files) >= 2: # 파일이 여러 개면 앞/뒤로 과거/현재 자동 구분
...
if 'today_df' in st.session_state:
    ...
    st.markdown(render_master_table(curr_curr := curr, prev_prev := prev, title="🔔 3. 판도 변화 (BAR 변경 알림)", mode="판도변화"), unsafe_allow_html=True)
