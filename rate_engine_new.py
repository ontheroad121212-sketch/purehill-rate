...
#
# 칸 간격은 두 구간으로 나뉩니다.
#   B0pp ~ B10 (칸 1~13)  간격 8%   — 앵커·페이스·재고가 실제로 쓰는 구간
#   B11 ~ B13  (칸 14~16) 간격 2.5% — 위기 대응 헤드룸 (RM 승인 전용)
#
# 왜 아래쪽만 압축했나
#   자동 하향은 B10(NEW_TYPE_RUNG_MAX)에서 멈춥니다. 앵커도 B2~B10만 씁니다.
#   그래서 B11~B13은 승인 없이는 들어갈 수 없는 칸인데, 여기까지 8%를 계속
#   적용하면 표의 최저가가 구 요금표 최저가(HDT BAR8 250,000원)보다 낮아집니다.
#   쓰지도 않을 최저가가 요금표에 남으면 채널·제휴 협상에서 근거로 잡힙니다.
#   2.5%로 압축하면 표의 바닥이 구 요금표 수준(HDT 252,000원)에서 멈춥니다.
#
#   ※ 믹스가중 BAR(100억 목표)는 앵커가 쓰는 칸으로만 계산되므로
#     이 압축은 목표 달성에 영향이 없습니다 (418,881원 / 목표 418,899원).
#
...
NEW_DIFF_RULE = [
    # 엠버 트윈 → 파인 더블 45,000원.
    #   "2~3만원 차이" 는 고객이 내는 값 기준입니다. 평균 할인 25% 를 감안하면
    #   30,000원 BAR 차이는 실제로 22,500원밖에 안 보입니다. 45,000원이면
    #   할인 후 33,750원 — 의도한 폭이 손님 눈에 그대로 남습니다.
    ("HDP", "HDT", 45000, 0.100),
    # 파인 더블 → 포레스트 가든 더블 75,000원 (현재 실제 차이 54,920원).
    #   '포레스트를 확실히 더 프리미엄으로' 결정에 맞춰 격차를 37% 넓힙니다.
    ("FDB", "HDP", 75000, 0.180),
    # ※ 포레스트 가든 더블 → EB 는 '엑스트라베드' 차이뿐입니다.
    #   현장에서 1박 88,000원(연박 인추비 33,000원)만 내면 같은 혜택을 받으므로,
    #   차액이 그보다 커지면 고객이 'FDB + 현장 추가'로 옮겨갑니다.
    #   대체 비용은 요금 수준과 무관한 고정값이라 정률을 쓰지 않고 정액으로 못박습니다.
    #   금액은 85,000원입니다. 현장 추가 88,000원 바로 아래라 대체 유인이
    #   3,000원밖에 안 남습니다 — 프리미엄을 최대로 받되 역전은 안 나는 지점입니다.
    #   (아래 대체 상한 88,000원이 이 선을 넘지 않게 지킵니다.)
    ("FDE", "FDB", 85000, 0.000),
    # 포레스트 플로라도 '가든 더블 + 65,000원' 으로 EB 와 같은 금액에 둡니다.
    #   EB 와 플로라는 같은 가격대의 대체재라 서로 싸움을 붙일 이유가 없고,
    #   둘 중 어느 쪽이 남아도 고객이 값 때문에 갈아타지 않게 하려는 것입니다.
    #   그래서 기준을 EB 가 아니라 '가든 더블'로 잡아 두 객실이 항상 동일가입니다.
    ("FFD", "FDB", 85000, 0.000),
    ("HDF", "FDE", 75000, 0.150),
    # 포레스트 펫 더블은 플로라보다 확실히 위에 둡니다.
    #   반려동물 동반 비용은 본질적으로 정액이라 정액(50,000)을 축으로 잡고,
    #   한 칸 간격(7.4%)보다 큰 정률 10% 로 성수기 역전을 막습니다.
    ("FPT", "FFD", 50000, 0.100),
]

# =============================================================================
# 3-C. 대체 상한 (arbitrage cap) — 차액이 '현장 추가 비용'을 넘지 않게
# -----------------------------------------------------------------------------
# 역전방지 계단이 차액의 '하한'을 지킨다면, 이건 '상한'을 지킵니다.
#
# 왜 필요한가
#   두 객실의 차이를 고객이 현장에서 돈으로 메울 수 있으면, 요금 차액은
#   그 비용을 넘을 수 없습니다. 넘으면 고객은 싼 객실을 잡고 현장에서 추가합니다.
#   그러면 상급 객실은 안 팔리고, 우리는 차액 대신 추가 요금만 받습니다.
#
#   포레스트 가든 더블 → EB 가 정확히 그 관계입니다 (엑스트라베드 차이뿐).
#   요금표 차액은 55,000원으로 못박았지만, 타입 조정으로 EB 칸이 올라가면
#   차액이 벌어집니다. 그때 이 상한이 되돌립니다.
#
# 언제 발동하는가 — 계단과 정확히 대칭입니다
#   · 하급에 팔 재고가 있다  → 대체가 가능하니 상한을 강제 (상급 칸을 내림)
#   · 하급이 거의 소진됐다   → 대체할 수 없으니 상한 해제 (상급은 자유롭게 상향)
#
# (상급, 기준 객실, 최대 차액, 설명)
# =============================================================================
NEW_CAP_RULE = [
    ("FDE", "FDB", 88000, "엑스트라베드 현장 추가 1박 88,000원 (연박 인추비 33,000원)"),
]
NEW_CAP_ON = True
NEW_RUNG = {
    1: 737000, 2: 682000, 3: 632000, 4: 585000, 5: 542000, 6: 502000, 7: 464000, 8: 430000,
    9: 398000, 10: 369000, 11: 341000, 12: 316000, 13: 293000, 14: 286000, 15: 279000, 16: 272000,
}
...
# 타입별 16칸 요금표 — 로드(BAR) 기준가.
#
# ※ 2026-09-18 레벨 정정 (x1.08) — 왜 올렸나
#   확정판은 "지금 실제로 걸고 있는 BAR" 대비 +5.0% 였는데, 그 인상이
#   객실박 3% 짜리 객실들(펫 +34% · 풀빌라 +61% · 플로라 +35%)에 몰려 있고
#   정작 객실박 42% 를 먹는 엠버 트윈(-3.8%) · 파인 더블(+5.7%)은 제자리였습니다.
#   실제 점유율도 FIT 추출만 보고 낮게 잡았던 것으로, 재고 리포트 기준
#   9월 65.8% · 10월 62.4% · 113일 중 70% 이상이 28일입니다.
#   → 레벨 x1.08 로 올려 엠버 트윈 앵커 평균이 351,772 → 365,391 (+3.9%) 가 됩니다.
#     믹스가중 BAR 445,765 (지금 399,016 대비 +11.7%).
#
# ※ 펫 더블 배수 정정 (2.015 → 1.504)
#   배수를 실현 ADR 로 회귀했는데 할인율이 객실마다 3.5%~27.9% 로 달라서,
#   할인을 거의 안 주던 객실의 BAR 이 부풀었습니다. 펫은 실제 BAR 위치로
#   되돌렸습니다. 포레스트 3종이 높은 것은 '프리미엄으로 판다'는 결정대로입니다.
NEW_TABLE = {
    "FDB": {"0pp": 957000, "0p": 885000, "0": 820000, "1": 760000, "2": 703000, "3": 651000, "4": 602000, "5": 560000, "6": 523000, "7": 489000, "8": 461000, "9": 436000, "10": 413000, "11": 406000, "12": 399000, "13": 392000},
    "FDE": {"0pp": 1042000, "0p": 970000, "0": 905000, "1": 845000, "2": 788000, "3": 736000, "4": 687000, "5": 645000, "6": 608000, "7": 574000, "8": 546000, "9": 521000, "10": 498000, "11": 491000, "12": 484000, "13": 477000},
    "HDP": {"0pp": 811000, "0p": 750000, "0": 695000, "1": 644000, "2": 596000, "3": 552000, "4": 510000, "5": 475000, "6": 443000, "7": 414000, "8": 386000, "9": 361000, "10": 338000, "11": 331000, "12": 324000, "13": 317000},
    "HDT": {"0pp": 737000, "0p": 682000, "0": 632000, "1": 585000, "2": 542000, "3": 502000, "4": 464000, "5": 430000, "6": 398000, "7": 369000, "8": 341000, "9": 316000, "10": 293000, "11": 286000, "12": 279000, "13": 272000},
    "HDF": {"0pp": 1198000, "0p": 1116000, "0": 1041000, "1": 972000, "2": 906000, "3": 846000, "4": 790000, "5": 742000, "6": 699000, "7": 660000, "8": 628000, "9": 599000, "10": 573000, "11": 566000, "12": 559000, "13": 552000},
    "GDB": {"0pp": 638000, "0p": 599000, "0": 562000, "1": 527000, "2": 495000, "3": 465000, "4": 436000, "5": 409000, "6": 384000, "7": 360000, "8": 338000, "9": 318000, "10": 298000, "11": 298000, "12": 298000, "13": 298000},
    "GDF": {"0pp": 1049000, "0p": 966000, "0": 889000, "1": 818000, "2": 753000, "3": 693000, "4": 638000, "5": 587000, "6": 541000, "7": 498000, "8": 458000, "9": 422000, "10": 388000, "11": 388000, "12": 388000, "13": 388000},
    "FFD": {"0pp": 1042000, "0p": 970000, "0": 905000, "1": 845000, "2": 788000, "3": 736000, "4": 687000, "5": 645000, "6": 608000, "7": 574000, "8": 546000, "9": 521000, "10": 498000, "11": 491000, "12": 484000, "13": 477000},
    "FPT": {"0pp": 1146000, "0p": 1067000, "0": 996000, "1": 930000, "2": 867000, "3": 810000, "4": 756000, "5": 710000, "6": 669000, "7": 631000, "8": 601000, "9": 573000, "10": 548000, "11": 541000, "12": 534000, "13": 527000},
    "PPV": {"0pp": 2790000, "0p": 2676000, "0": 2566000, "1": 2461000, "2": 2361000, "3": 2264000, "4": 2171000, "5": 2083000, "6": 1997000, "7": 1916000, "8": 1837000, "9": 1762000, "10": 1690000, "11": 1290000, "12": 1290000, "13": 1290000},
}

# =============================================================================
# 1-B. 요금 하한 (price floor) — 이 객실은 이 밑으로 내려가지 않습니다
# -----------------------------------------------------------------------------
# 사다리·페이스·재고가 뭐라고 하든 로드(BAR) 요금이 이 값 아래로 못 갑니다.
# Scarcity Floor 가 '칸'의 하한이라면, 이건 '금액'의 하한입니다.
#
# 왜 칸이 아니라 금액인가
#   칸 하한은 타입마다 금액이 달라집니다. "그린밸리는 무조건 298,000원"처럼
#   상품 자체의 최저 판매가가 정해져 있으면 금액으로 못박는 게 맞습니다.
#
# 무엇을 막는가 — 사다리 아래쪽에서 값이 눌리는 구간입니다.
#   그린밸리 디럭스 더블 298,000원 → B7 부터 평평 (B7~B13 동일)
#   그린밸리 디럭스 패밀리 388,000원 → B10 부터 평평
#       (= 298,000 + 그린밸리 계단 최소 간격 90,000. 계단이 실제로 팔 값과
#         요금표가 어긋나지 않게 파생시킵니다)
#   프라이빗 풀 빌라 1,100,000원 → B7 부터 평평
#
# 부작용 — 알고 쓰셔야 합니다
#   평평한 구간에서는 칸을 내려도 요금이 안 내려갑니다. 즉 그 객실의
#   페이스·재고 하향 신호가 요금에 반영되지 않습니다 (하한이 이긴 것).
#   화면에서는 '요금 하한' 배지로 표시합니다.
#
# 하한은 로드(BAR) 기준가에 걸립니다. 회원가·프로모션은 그 하한가에서
# 계수를 곱하므로 노출가는 더 낮아질 수 있습니다 (기존 앱과 동일한 취급).
# =============================================================================
NEW_PRICE_FLOOR = {
    "GDB": 298000,      # 그린밸리 디럭스 더블 — 최저 판매가 확정
    "PPV": 1290000,     # 프라이빗 풀 빌라 — 조식 포함, 구 요금표 BAR8 과 동일선
}

# 자동 하한 — 엔진이 '자동으로는' 이 밑으로 안 내려갑니다.
#   절대 하한(NEW_PRICE_FLOOR)과 다릅니다. 절대 하한은 RM 승인이 있어야 닿는
#   B11~B13 의 바닥이고, 자동 하한은 B10(자동 하향 한계)에 놓입니다.
#
#   풀빌라는 호텔 유일의 스위트입니다. 1실짜리 수요가 132실 호텔의 수요 곡선을
#   따라 바닥까지 끌려 내려갈 이유가 없습니다. 평소엔 1,690,000~2,790,000 띠
#   안에서만 움직이고, 129만원은 '승인하면 닿는 바닥'으로 남겨 둡니다.
#   → B10 1,690,000 에서 B11 1,290,000 으로 떨어지는 계단은 의도된 것입니다.
#     그 아래로 내리려면 사람이 승인해야 한다는 뜻입니다.
NEW_PRICE_FLOOR_AUTO = {
    "PPV": 1690000,
}

# 객실에 붙는 상품 조건 (요금이 아니라 '무엇이 포함되는가'). 화면 표시용입니다.
NEW_ROOM_NOTE = {
    "PPV": "조식 포함 · 최저 1,290,000원부터",
}
...
# 소량 객실 — 자기 잔여율로 칸을 움직이지 않습니다.
#
# 왜
#   잔여율은 1/전체 단위로만 튑니다. 2실짜리는 100% / 50% / 0% 셋뿐이고
#   1실짜리는 100% / 0% 둘뿐입니다. 타입 조정의 임계값(15% / 30%)을
#   해상할 수가 없습니다 — 상향 조건(<=15%)에 걸리려면 전체가 7실 이상이어야
#   합니다 (1/7 = 14.3%). 그래서 6실 이하 객실은 '여유(>=85%) → 2칸 하향'과
#   '매진' 사이를 오갈 뿐, 중간이 없습니다. 실측에서 플로라가 하루 만에
#   627,000 → 822,000 으로 튀던 것이 이 때문입니다.
#
#   더 근본적으로, 1~2실의 판매 여부는 수요의 '신호'가 아니라 '잡음'입니다.
#   한 건 팔렸다는 사실에서 시장에 대해 알 수 있는 게 없습니다.
#
# 그래서
#   6실 이하 객실은 날짜 신호(앵커 · 연휴 · 페이스 · 호텔 재고 · floor)만
#   따라갑니다. 자기 재고는 '판매 마감' 판정에만 씁니다.
#   호텔 전체가 희소해지면(총잔여 20% 이하) 주는 1칸 상향은 그대로 받습니다 —
#   그건 자기 재고가 아니라 호텔 수요 신호이기 때문입니다.
#   서열 계단 · 대체 상한 · 요금 하한도 평소대로 적용됩니다.
NEW_SMALL_CAP = 6

# 재고 풀링 — 표본이 작은 객실은 '같은 값에 파는 형제 객실'과 재고를 합쳐 봅니다.
#
# 왜
#   플로라는 2실이라 혼자서는 신호가 안 나옵니다. 그런데 포레스트 가든 더블 EB
#   (8실)와 요금표상 완전히 같은 값이고 같은 티어입니다. 두 객실은 고객 입장에서
#   같은 가격대의 대체재이므로, 재고를 합쳐 10실짜리 한 덩어리로 보는 게
#   각자 따로 보는 것보다 실제 수요를 훨씬 잘 반영합니다.
#
# 효과
#   ① 2실짜리의 잡음이 사라집니다 (10실 표본).
#   ② 두 객실이 항상 같은 칸 = 항상 같은 금액이 됩니다. 요금표가 같으니
#      적용가도 같아야 한다는 요청에 그대로 맞습니다.
#   ③ 판매 마감은 여전히 객실별로 판정합니다 — 플로라가 다 나갔으면
#      플로라만 닫히고, 요금은 티어 신호를 계속 따릅니다.
#
# 풀에 들어간 객실은 소량 객실 규칙 대신 이 규칙을 씁니다.
NEW_TYPE_POOL = [("FDE", "FFD")]
NEW_POOL_OF = {rt: grp for grp in NEW_TYPE_POOL for rt in grp}
...
NEW_LADDER_GAP = {
    "HDP": 45000,   # 힐 엠버 트윈 → 힐 파인 더블          (차액 규칙과 동일)
    "FDB": 75000,   # 힐 파인 더블 → 포레스트 가든 더블
    "FDE": 85000,   # 포레스트 가든 더블 → EB (차액 규칙과 동일)
    "FFD": 85000,   # 포레스트 가든 더블 → 플로라 (EB 와 동일가)
    "HDF": 30000,   # [EB · 플로라] 티어 → 힐 루나 패밀리
}
# 펫 더블은 [EB · 플로라] 티어 바로 위입니다. 다만 메인 체인에 끼워 넣으면
# 2실짜리 펫이 18실짜리 루나 패밀리를 끌고 올라가므로 별도 계층으로 둡니다.
NEW_LADDER_CHAIN_PET = ["FFD", "FPT"]
NEW_LADDER_GAP_PET = {"FPT": 50000}
# 이 계층은 '잠식 방지'가 아니라 '포지셔닝'입니다 — 펫은 언제나 플로라보다
# 비싸야 합니다. 그래서 하급(플로라)이 매진이어도 계단을 풀지 않습니다.
#   일반 계단은 "하급이 매진이면 잠식당할 재고가 없으니 제외"가 맞지만,
#   플로라는 2실뿐이라 매진이 예외가 아니라 일상입니다. 그 규칙을 그대로 두면
#   113일 중 12일에서 펫이 플로라보다 싸게 찍힙니다 (실측).
NEW_LADDER_PET_ALWAYS = True

# 그린밸리는 펜션형으로 상품군이 달라 별도 계층을 씁니다.
NEW_LADDER_CHAIN_GV = ["GDB", "GDF"]
NEW_LADDER_GAP_GV = {"GDF": 90000}
# 하급(GDB)이 하한에 눌려 있으면 상급도 계단만큼은 받쳐줘야 요금표와
# 계단이 어긋나지 않습니다. 그래서 GDF 하한은 손으로 적지 않고 파생시킵니다.
NEW_PRICE_FLOOR.setdefault("GDF", NEW_PRICE_FLOOR["GDB"] + NEW_LADDER_GAP_GV["GDF"])
...
def new_load_price(rt, rung):
    ...
    요금표에는 이미 하한이 반영되어 있지만, 표를 손으로 고쳐도 하한이
    깨지지 않도록 여기서 한 번 더 막습니다 (요금표가 유일한 원장이고
    하한은 그 원장의 불변식입니다).
    """
    r = max(1, min(NEW_N, int(rung)))
    v = NEW_TABLE.get(rt, {}).get(NEW_LAB[r - 1], 0)
    if not v:
        return 0
    v = max(int(v), NEW_PRICE_FLOOR.get(rt, 0))
    if r <= NEW_TYPE_RUNG_MAX:        # 자동 구간에만 자동 하한이 걸립니다
        v = max(v, NEW_PRICE_FLOOR_AUTO.get(rt, 0))
    return v


def new_floor_hit(rt, rung):
    """이 칸에서 요금 하한이 실제로 구속 중인가 (화면 배지용)."""
    fl = NEW_PRICE_FLOOR.get(rt)
    if not fl:
        return False
    return new_load_price(rt, rung) <= fl
...
def new_apply_ladder(rows):
    ...
    if not NEW_LADDER_ON:
        return moved
    for chain, gaps, always in (
            (NEW_LADDER_CHAIN, NEW_LADDER_GAP, False),
            (NEW_LADDER_CHAIN_PET, NEW_LADDER_GAP_PET, NEW_LADDER_PET_ALWAYS),
            (NEW_LADDER_CHAIN_GV, NEW_LADDER_GAP_GV, False)):
        ...
        for step in chain:
            ...
            if last is not None:
                ...
                if f is None and always:
                    f = 1.0          # 포지셔닝 계층 — 매진이어도 간격 전액 강제
                if f is not None:
                    for rt, r in here:
            ...
            for rt, r in here:
                av = _num(r.get('avail'))
                if always or av is None or av > 0:
                    live.append((rt, r))
            if live:
    ...
    # ── 대체 상한: 차액이 '현장 추가 비용'을 넘으면 상급 칸을 내립니다 ──
    #   하급이 거의 소진되면 대체가 불가하므로 상한을 걸지 않습니다.
    if NEW_CAP_ON:
        for up, base, cap, _why in NEW_CAP_RULE:
            ru, rb = rows.get(up), rows.get(base)
            if ru is None or rb is None:
                continue
            if _ladder_factor(rb.get('avail'), rb.get('cap')) is None:
                continue          # 하급 소진 → 대체 불가 → 상한 해제
            ceil = new_load_price(base, rb['rung']) + cap
            rung, dn = int(ru['rung']), 0
            while rung < NEW_TYPE_RUNG_MAX and new_load_price(up, rung) > ceil:
                rung += 1
                dn += 1
            if dn:
                ru['rung'] = rung
                moved[up] = moved.get(up, 0) - dn
    return moved
...
def new_compute_types(curr_df, day_df, today=None, overrides=None):
    ...
    # 재고 풀 합계 — (날짜, 풀) → (잔여 합, 전체 합)
    pool_tot = {}
    if NEW_TYPE_POOL:
        for _, pr in src.iterrows():
            prt = str(pr['RoomID']).strip().upper() if pr['RoomID'] is not None else ""
            grp = NEW_POOL_OF.get(prt)
            if not grp:
                continue
            pav, pcap = _num(pr.get('Available')), _num(pr.get('Total'))
            if pcap is None or pcap <= 0:
                continue
            k = (pr['Date'], grp)
            a0, c0 = pool_tot.get(k, (0.0, 0.0))
            pool_tot[k] = (a0 + (pav or 0), c0 + pcap)
    rows = []
    for _, r in src.iterrows():
        ...
        tadj, state, why = 0, "판매", ""
        # 풀에 속하면 티어 합계로 잔여율을 봅니다 (마감 판정은 자기 재고로).
        grp = NEW_POOL_OF.get(rt)
        pooled = False
        if grp:
            pv = pool_tot.get((d, grp))
            if pv and pv[1] > 0:
                remt, pooled = pv[0] / pv[1], True
        small = (not pooled) and cap is not None and 0 < cap <= NEW_SMALL_CAP
        if av is None or cap is None or cap <= 0:
            state, why = "데이터 이상", "잔여/전체 결측 — 날짜 칸을 그대로 씁니다"
        elif av <= 0:
            state = "마감" if av == 0 else "오버부킹"
            if pooled:
                # 자기 재고는 없지만 티어에는 남아 있을 수 있습니다.
                # 요금은 티어 신호를 그대로 따르고, 판매만 닫습니다.
                for thr, st in NEW_TYPE_TIGHT:
                    if remt is not None and remt <= thr:
                        tadj = st
                        break
                why = ("팔 재고 없음 — 요금은 티어(%s) 합계 잔여 %s 기준"
                       % (" · ".join(grp), _pct(remt)))
            elif small:
                why = ("팔 재고 없음 — %d실짜리라 잔여율이 신호가 못 되어 "
                       "날짜 칸을 그대로 씁니다" % int(cap))
            elif NEW_TYPE_SOLDOUT_TIGHT:
            ...
            why = ("%d실 객실 — 잔여율이 임계값을 해상 못 해 타입 조정을 "
                   "쓰지 않습니다 (날짜 신호만)" % int(cap))
        else:
            hit = False
            for thr, s in NEW_TYPE_TIGHT:
                if remt is not None and remt <= thr:
                    tadj, hit = s, True
                    why = ("티어(%s) 잔여 %d%% 이하" % (" · ".join(grp), int(thr * 100))
                           if pooled else "타입 잔여 %d%% 이하" % int(thr * 100))
                    break
        ...
        if small and av is not None and av > 0 \
                and remh is not None and remh <= NEW_TYPE_SCARCE_REMH:
            tadj, why = -1, "호텔이 거의 매진 — 소량 객실도 1칸 상향"
    ...
    for r in rows:
        ...
        why = r['why']
        if up > 0:
            why = (why + " · " if why else "") + "역전방지 계단 %d칸 상향" % up
        elif up < 0:
            why = (why + " · " if why else "") + "대체 상한 %d칸 하향" % -up
        out.append(dict(
...
""", unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.caption("칸 간격 B0pp~B10 = 8%, B11~B13 = 2.5%(위기 헤드룸). 왼쪽이 비쌉니다. "
               "B11~B13은 자동 진입 금지 구간(승인 항목)입니다.")
...
def _new_tab_final(day_df, type_df, ov_raw=None):
    ...
    for rt in order:
        ...
        for i, d in enumerate(dates):
            ...
            if bool(row['is_override']):
                ...
                mk += "★"
            lu = int(row.get('ladder_up', 0) or 0)
            if lu > 0:
                mk += "⇧"
            elif lu < 0:
                mk += "⇩"
            if new_floor_hit(rt, int(row['rung'])):
                mk += "⊥"
    ...
    st.caption("셀 = 요금 / 칸 / 잔여. **점선 테두리 + '마감·오버부킹' 배지**가 붙은 셀은 "
               ...
               "★ RM 승인 구간 · ✋ 수동 예외 · ⇧ 역전방지 계단 상향 · "
               "⇩ 대체 상한 하향 · ⊥ 요금 하한에 눌림(칸을 더 내려도 요금이 그대로).")
...
def _new_tab_ratecard():
    ...
    st.caption("칸 간격 B0pp~B10 = 8%, B11~B13 = 2.5%(위기 헤드룸 · 승인 전용). "
               "왼쪽이 비쌉니다. 헤더 아래 숫자는 칸 인덱스(1~16)입니다.")
    _dl(f"📥 요금표 엑셀 (서식 유지) — {view}",
        [{'sheet': '타입별 요금표', 'title': f"객실타입별 16칸 요금표 · {view}",
          ...
          'rows': xrows, 'cw': 11,
          'note': "칸 간격 B0pp~B10 = 8%, B11~B13 = 2.5%(승인 전용). "
                  "왼쪽이 비쌉니다. 배수는 HDT=1.0 기준(B7 시점)입니다."},
    ...
    for i, step in enumerate(NEW_LADDER_CHAIN):
        ...
        prev_mem = mem
    chain_rows.append({"계층": "별도 · FFD 포레스트 플로라 더블 → FPT 펫 더블 "
                             "[포지셔닝 — 매진이어도 항상 강제]",
                       "최소 간격": f"{NEW_LADDER_GAP_PET['FPT']:,}원",
                       "같은 칸 실제 간격":
                           f"{min(new_load_price('FPT', j) - new_load_price('FFD', j) for j in range(1, NEW_N+1)):,}"
                           f" ~ {max(new_load_price('FPT', j) - new_load_price('FFD', j) for j in range(1, NEW_N+1)):,}원"})
    ...
    st.caption("화면·엑셀에서 계단으로 올라간 셀은 \u21e7 로 표시되고 '근거' 열에 "
               "'역전방지 계단 N칸 상향'이 들어갑니다.")

    # ── ⑥ 대체 상한 ───────────────────────────────────────────────
    st.markdown(_mx_title("⑥ 대체 상한 — 차액이 '현장 추가 비용'을 넘지 않게"),
                unsafe_allow_html=True)
    st.markdown(
        "<div class='renote warn'>계단이 차액의 <b>하한</b>을 지킨다면, 이건 "
        "<b>상한</b>을 지킵니다.<br><br>"
        "두 객실의 차이를 고객이 <b>현장에서 돈으로 메울 수 있으면</b>, 요금 차액은 "
        "그 비용을 넘을 수 없습니다. 넘으면 고객은 싼 객실을 잡고 현장에서 추가합니다 — "
        "상급은 안 팔리고 우리는 차액 대신 추가 요금만 받습니다.<br><br>"
        "<b>가든 더블 → EB</b> 가 정확히 그 관계입니다 (엑스트라베드 차이뿐). "
        "발동 기준은 계단과 정확히 대칭입니다 — <b>하급에 팔 재고가 있으면</b> 상한을 "
        "강제하고(상급 칸을 내림), <b>하급이 소진되면</b> 대체가 불가하니 상한을 "
        "풉니다.</div>", unsafe_allow_html=True)
    cap_rows = []
    for up, base, cap, why in NEW_CAP_RULE:
        nat = [new_load_price(up, j) - new_load_price(base, j)
               for j in range(1, NEW_N + 1)]
        cap_rows.append({
            "객실": f"{up} {NEW_ROOM_NAMES[up]}",
            "기준 객실": f"{base} {NEW_ROOM_NAMES[base]}",
            "최대 차액": f"{cap:,}원",
            "요금표 차액": f"{min(nat):,} ~ {max(nat):,}원",
            "여유": f"{cap - max(nat):,}원",
            "근거": why})
    cap_df = pd.DataFrame(cap_rows)
    st.dataframe(cap_df, use_container_width=True, hide_index=True)
    st.caption("여유 = 최대 차액 − 요금표 최대 차액. 이 만큼은 타입 조정으로 "
               "칸이 어긋나도 상한에 안 걸립니다. 상한에 걸려 내려간 셀은 '근거' 열에 "
               "'대체 상한 N칸 하향'이 들어갑니다. "
               f"{'상한 적용 중' if NEW_CAP_ON else '⚠ 현재 상한 OFF (NEW_CAP_ON=False)'}.")

    # ── ⑦ 요금 하한 ───────────────────────────────────────────────
    st.markdown(_mx_title("⑦ 요금 하한 — 이 금액 밑으로는 안 내려갑니다"),
                unsafe_allow_html=True)
    st.markdown(
        "<div class='renote'>Floor(⑤ 위 표)가 <b>칸</b>의 하한이라면, 이건 "
        "<b>금액</b>의 하한입니다. 상품 자체의 최저 판매가가 정해져 있는 객실에 씁니다.<br><br>"
        "<b>알고 쓰셔야 하는 부작용</b> — 하한에 눌린 칸에서는 칸을 내려도 요금이 "
        "안 내려갑니다. 즉 그 객실의 페이스·재고 <b>하향 신호가 요금에 반영되지 "
        "않습니다</b>. 대신 상향은 정상 작동합니다.<br>"
        "하한은 <b>로드(BAR) 기준가</b>에 걸립니다 — 회원가·프로모션은 그 하한가에서 "
        "계수를 곱하므로 노출가는 더 낮아질 수 있습니다 (기존 앱과 동일).</div>",
        unsafe_allow_html=True)
    pf_rows = []
    for r in NEW_ROOMS:
        fl = NEW_PRICE_FLOOR.get(r)
        if not fl:
            continue
        hit = [j for j in range(1, NEW_N + 1) if new_floor_hit(r, j)]
        pf_rows.append({
            "객실": f"{r} {NEW_ROOM_NAMES[r]}",
            "요금 하한": f"{fl:,}원",
            "눌리는 칸": f"{new_lab(min(hit))} ~ {new_lab(max(hit))} ({len(hit)}칸)" if hit else "—",
            "하한 위 최저가": f"{new_load_price(r, min(hit) - 1):,}원" if hit and min(hit) > 1 else "—",
            "비고": ("GDB 하한 + 그린밸리 계단 90,000원에서 파생" if r == "GDF"
                   else NEW_ROOM_NOTE.get(r, "확정 최저 판매가"))})
    if pf_rows:
        st.dataframe(pd.DataFrame(pf_rows), use_container_width=True, hide_index=True)
    else:
        st.info("설정된 요금 하한이 없습니다.")
    pf_df = pd.DataFrame(pf_rows)
    st.caption("자동 하향은 B10에서 멈추므로, 표에서 B11~B13 이 하한에 눌려 있어도 "
               "실제 운영에는 영향이 없습니다. 하한을 고치려면 "
               "rate_engine_new.py 상단 NEW_PRICE_FLOOR 를 바꾸십시오 — "
               "요금표(NEW_TABLE)와 계산 양쪽에 동시에 반영됩니다.")
    ...
    with pd.ExcelWriter(buf, engine='openpyxl') as w:
        ...
        cap_df.to_excel(w, index=False, sheet_name="대체 상한")
        if not pf_df.empty:
            pf_df.to_excel(w, index=False, sheet_name="요금 하한")
...
def _new_tab_types(day_df, type_df):
    ...
    st.markdown(_mx_title(f"객실타입별 확정 칸 · {mode} — 행 = 객실 · 열 = 날짜", True),
                unsafe_allow_html=True)
    st.markdown(_legend("<b>⇧</b> 역전방지 계단 상향 · <b>⇩</b> 대체 상한 하향 · "
                        "<b>⊥</b> 요금 하한에 눌림"), unsafe_allow_html=True)
    ...
    st.caption("**색은 칸의 절대 위치입니다** — 비쌀수록 진한 빨강, 쌀수록 연한 초록. "
        ...
        "(요금은 회원 노출가로 계속 표시됩니다). "
               ...
               "계단을 걸지 않습니다. **⇩ 는 대체 상한으로 내려간 셀**(차액이 현장 추가 "
               "비용을 넘지 않게), **⊥ 는 요금 하한에 눌린 셀**(칸을 더 내려도 요금이 "
               "그대로)입니다. 이전 대비 변화는 2번 탭에서 보십시오.")
